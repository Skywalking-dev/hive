#!/usr/bin/env bash
# xAI Grok API handler for Hive.
# OAI-compatible. 2M context window, $25 free credit.
# Models: grok-4-1-fast (2M, cheapest), grok-4 (256K), grok-4-20 (2M)
set -euo pipefail
...
[[ -n "${XAI_API_KEY:-}" ]] || die "XAI_API_KEY not set"
...
cmd_ask() {
  local query="" model="grok-4-1-fast" system="" temperature=0.7 max_tokens="" json_mode=false stream=false
...
# --- Main ---
command="${1:-}"
shift || die "Usage: xai_handler.sh <ask|models> <query> [options]"
